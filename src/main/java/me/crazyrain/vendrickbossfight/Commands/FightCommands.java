...
import me.crazyrain.vendrickbossfight.inventories.FightStatsGUI;
        ...
        if (cmd.getLabel().equalsIgnoreCase("venfight")){
            ...
            if (args.length > 0){
                if (args[0].equalsIgnoreCase("help")){
                    ...
                    player.sendMessage(ChatColor.AQUA + "Distortion - n: No distortion, f: Flaming, t: Tidal, s: Stormy");
                    player.sendMessage("");
                    ...
                    if (StatsTracker.isViewExpired()){
                        player.sendMessage(venPrefix + ChatColor.RED + " This action has expired!");
                    } else {
                        FightStatsGUI gui = new FightStatsGUI(player.getUniqueId(), StatsTracker.getPlayers(), StatsTracker.getLost());
